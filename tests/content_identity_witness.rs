...
//! representative EncodedEthos value under the current layout version.
//!
...
//! the archived representation of a type reachable from `EncodedEthos` (a new or
...
//!
//! If this test fails, the archived representation of a EncodedEthos value changed.
//! That is a layout event, never a casual edit: bump `EncodedEthosDomain`'s
...
use core_ethos::{EncodedDeclaration, EncodedEthos, EncodedEthosDomain, EncodedReference};
use name_table::Identifier;

/// The content identity of a representative single-declaration Ethos value — a public
/// `Newtype` over `Boolean` at a fixed identifier index — under the current
/// EncodedEthos layout, as a lowercase hex blake3 digest. Pinned at layout 5, where
/// identifiers are namespace variants carrying `u16` locals and Ethos values carry
/// ordered streaming relations. The NameTable remains excluded from the pre-image.
const REPRESENTATIVE_ETHOS_IDENTITY_LAYOUT_5: &str =
    "810b6fa336618ee9c229779edbe71a9b90497d8ebbf346fa7d16c58d9c74cc07";

/// The representative value the constant pins: an Ethos value of one public declaration,
...
fn representative_ethos() -> EncodedEthos {
    EncodedEthos::new(vec![EncodedDeclaration::public(EncodedType::Newtype(
        EncodedNewtype::new(Identifier::Schema(0), EncodedReference::Boolean),
    ))])
}

#[test]
fn representative_ethos_identity_is_pinned_under_the_current_layout() {
    ...
    assert_eq!(
        EncodedEthosDomain::layout_version().value(),
        5,
        "the witnessed layout version moved; re-derive the pinned hash deliberately",
    );

    let identity = representative_ethos()
        .content_identity()
        .expect("content identity");

    assert_eq!(
        identity.to_hexadecimal(),
        REPRESENTATIVE_ETHOS_IDENTITY_LAYOUT_5,
        "the archived representation of a EncodedEthos value changed — this is a layout \
         event: bump EncodedEthosDomain's LayoutVersion in src/declaration.rs, document \
